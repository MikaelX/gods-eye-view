...
| **Trafikverket Trafikinfo (Camera + TravelTimeRoute + Situation)** | Swedish road CCTV stills + street congestion segments + traffic events (Stockholm CountyNo 1 default; other counties / nationwide via `TRAFIKVERKET_COUNTY_NOS`) | Free API key; [Trafikinfo](https://api.trafikinfo.trafikverket.se/) — attribution required | "Contains data from Trafikverket" |
...
- **Trafikverket (Sweden).** Optional pack — set `TRAFIKVERKET_API_KEY` (or paste via POWER UP). **Same key** unlocks (1) Active **Camera** stills and (2) Swedish street traffic overlays: **TravelTimeRoute** segments colored by `TrafficStatus` plus **Situation** events (`namespace="road.trafficinfo"` required). Default county pack is Stockholm (`CountyNo = 1`); set `TRAFIKVERKET_COUNTY_NOS` to a comma list (e.g. `1,12,14`) or `*` / `all` for nationwide (feature caps still apply — not a product hard-lock to Stockholm). CCTV: disable with `CCTV_TRAFIKVERKET_ENABLED=0`, cap with `CCTV_TRAFIKVERKET_MAX_SOURCES` (default 200). Traffic: `TRAFIKVERKET_TRAFFIC_ENABLED`, `TRAFIKVERKET_TRAFFIC_MAX_ROUTES` (default 250), `TRAFIKVERKET_SITUATION_ENABLED`, `TRAFIKVERKET_SITUATION_MAX_FEATURES` (default 400). **TrafficFlow** point sensors are reserved for a follow-up (`TRAFIKVERKET_TRAFFIC_FLOW_ENABLED` stub). **Account sync:** after registering a new Trafikinfo account, wait **30–60 minutes** before creating an API key — portal errors such as "User id not found" mean the user record is still syncing; wait and retry once, do not spam-create keys. Use `Content-Type: text/xml` (or `text/plain`) for POST requests, not `application/xml`. Placement uses `Geometry.WGS84` only (never SWEREF99TM). TomTom remains optional BYOK for global flow tiles outside Sweden. See `docs/trafikverket-cctv.md` (English + Swedish).
