...
| **Trafikverket Trafikinfo (full Sweden pack)** | Nationwide Camera + TravelTimeRoute + Situation + TrafficFlow + RoadCondition + WeatherMeasurepoint + TrafficSafetyCamera (ATK); optional `TRAFIKVERKET_COUNTY_NOS` narrow | Free API key; [Trafikinfo](https://api.trafikinfo.trafikverket.se/) — attribution required | "Contains data from Trafikverket" |
...
- **Trafikverket (Sweden).** Optional pack — set `TRAFIKVERKET_API_KEY` (or paste via POWER UP). **One key** unlocks the full national stack: Active **Camera** stills, **TravelTimeRoute** (primary street coloring), **Situation** (`namespace="road.trafficinfo"` required), **TrafficFlow** sensors, plus **RoadCondition**, **WeatherMeasurepoint**, and **TrafficSafetyCamera** (ATK POIs — labeled vs CCTV). **Default scope is nationwide Sweden** (no CountyNo filter). Optionally narrow with `TRAFIKVERKET_COUNTY_NOS` (e.g. `1` or `1,12,14`); `*` / `all` / unset = nationwide. Performance: TomTom-style **viewport bbox** for TravelTimeRoute/Situation/TrafficFlow (clamp+pad, ~120s server cache, ~90s poll); CCTV-style **nearest-N** + PhotoUrl-on-activation for Camera/ATK. Caps (env): cameras 500, routes 500, situations 1500, flow 1500, road 800, weather 400, ATK 400 — see `docs/trafikverket-cctv.md`. Disable with the matching `*_ENABLED=0` knobs. **Account sync:** after registering a new Trafikinfo account, wait **30–60 minutes** before creating an API key — portal errors such as "User id not found" mean the user record is still syncing; wait and retry once, do not spam-create keys. Use `Content-Type: text/xml` (or `text/plain`) for POST requests, not `application/xml`. Placement uses `Geometry.WGS84` only (never SWEREF99TM). TomTom remains optional BYOK for global flow tiles outside Sweden. Stockholm screenshots in docs are examples of the national pack.
