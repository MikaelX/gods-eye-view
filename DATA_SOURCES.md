...
| **Trafikverket Trafikinfo (Camera)** | CCTV camera catalog + stills, Stockholm county (CountyNo 1) | Free API key; [Trafikinfo](https://api.trafikinfo.trafikverket.se/) — attribution required | "Contains data from Trafikverket" |
...
- **Trafikverket Camera (Stockholm).** Optional pack — set `TRAFIKVERKET_API_KEY` (or paste via POWER UP). Loads Active cameras with `CountyNo = 1` (Stockholm) as stills (`feedType: image`) via the Trafikinfo Camera API + `PhotoUrl`, proxied like other CCTV packs. Disable with `CCTV_TRAFIKVERKET_ENABLED=0`. Cap with `CCTV_TRAFIKVERKET_MAX_SOURCES` (default 200). **Account sync:** after registering a new Trafikinfo account, wait **30–60 minutes** before creating an API key — portal errors such as "User id not found" mean the user record is still syncing; wait and retry once, do not spam-create keys. Use `Content-Type: text/xml` (or `text/plain`) for POST requests, not `application/xml`. Placement uses `Geometry.WGS84` only (never SWEREF99TM). See `docs/trafikverket-cctv.md` (English + Swedish).
